...
The repository is in active development. The landing page, seasonal themes, authenticated gallery, photo import and draft recovery, chronological replay, indexed Layered Memory overlay, and three bilingual curated routes are working. Authentication and production hardening remain planned work. See [ROADMAP.md](./ROADMAP.md) for the mergeable PR plan and release gates.
...
- The local curated gallery contains Philosopher's Path, Nakasendo, and Kumano Kodo, with ten bilingual, photographed, credited waypoint stories per route.
